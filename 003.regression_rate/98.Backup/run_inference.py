...
def run_inference(df, 
    ...
    # 4. 결과 데이터프레임 구축 및 역변환[cite: 3, 10]
    res_df = pd.DataFrame({
    ...
    for col in ['Actual', 'TFT_Prediction', 'LSTM_Prediction']:
        res_df_raw[col] = res_df_raw[col] * std_val + mean_val

    # t-1 시점 데이터 복원 및 방향성 분류
    ynum = var_list.index(target)
    y_t_minus_1_raw = (x_past[2][:, -1, ynum] * std_val) + mean_val
    ...
    for m_type in ['Actual', 'TFT', 'LSTM']:
        col_name = 'Actual' if m_type == 'Actual' else f'{m_type}_Prediction'
        res_df_raw[f'{m_type}_Move'] = res_df_raw.apply(lambda x: classify_movement(x[col_name], x['Actual_t_minus_1'], threshold), axis=1)
