...
class SimpleLSTM(nn.Module):
    def __init__(self, cfg: TFTConfig):
        ...
        self.lstm_layers = nn.ModuleList([
            nn.LSTM(cfg.past_vars if i == 0 else cfg.hidden_dim, 
                    cfg.hidden_dim, batch_first=True) 
            for i in range(2) # num_layers 고정 혹은 cfg에 추가
        ])
        
        final_dim = cfg.n_classes if cfg.output_mode == "multiclass" else cfg.output_dim
        self.fc = nn.Linear(cfg.hidden_dim, final_dim)

    def forward(self, x_past, x_known=None, x_static=None, pos=None):
        out = x_past
        ...
        return logits, {} # multiclass, regression은 로짓/값 그대로 반환