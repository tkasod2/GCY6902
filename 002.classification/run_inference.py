...
from sklearn.metrics import mean_squared_error, mean_absolute_error, accuracy_score, confusion_matrix, f1_score
...
def run_inference(df, 
    ...
    # 1. 데이터 준비 (학습 때와 동일한 전처리 적용)
    x_past, x_known, x_static, y, scaler, le = prepare_data(
        df, target=target, var_list=var_list, 
        trk_start_ym=trk_start_ym, seq_length=seq_length, ym_cut=ym_cut
    )
    ...
    with torch.no_grad():
        for x_p, x_k, x_s, p, y in ts_loader:
            ...
            all_tft_preds.append(torch.argmax(y_tft, dim=-1).numpy()) # ★argmax 처리로 클래스 예측값 저장
            all_lstm_preds.append(torch.argmax(y_lstm, dim=-1).numpy()) # ★argmax 처리로 클래스 예측값 저장
    ...
    f1_tft = f1_score(res_df_raw['Actual_Move'], res_df_raw['TFT_Move'], average='macro')
    f1_lstm = f1_score(res_df_raw['Actual_Move'], res_df_raw['LSTM_Move'], average='macro')

    print(f"TFT Accuracy: {acc_tft:.4f} | F1 Score: {f1_tft:.4f}")
    ...
    # 성능 기록을 위해 라벨 순서 고정
    sns.heatmap(cm_tft / (cm_tft.sum() + 1e-9), annot=True, fmt=".1%", cmap='Blues', ax=ax7,
    ...
    # 성능 기록을 위해 라벨 순서 고정
    sns.heatmap(cm_lstm / (cm_lstm.sum() + 1e-9), annot=True, fmt=".1%", cmap='Blues', ax=ax8,
            xticklabels=move_labels, yticklabels=move_labels)
    ...
    img_files = [
        (f"{target}_IMG2.png", 'A3'),
        (f"{target}_IMG3.png", 'A25'),
        (f"{target}_IMG4.png", 'A47')
    ]
