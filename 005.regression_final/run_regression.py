...
from models.dl.train_tft import SeqDataset, collate_fn, train, train_lstm
...
def fit_and_out(df,
                ...
                epochs=2,
                patience=1,
                trk_start_dt='2024030100',  # 시작 일자
                seq_length=21, # 7일
                dt_cut='2026030100',         # train/valid 분할 기준 일자
                threshold=0.02, # 변동 방향성 판단 기준 (예: 5% 이상 상승/하락)
                tgt_gap=3,
                seed=42,
    ...
    * 기타
    ...
    df_log_new = res_df_raw.copy()
    df_log_new['log'] = outdir
    
    log_path = os.path.join(project_path, 'outputs', 'df_log.csv.gz')
    try:
        if os.path.exists(log_path):
            df_log = pd.read_csv(log_path, encoding='utf-8')
            df_log = pd.concat([df_log, df_log_new], ignore_index=True)
        else:
            df_log = df_log_new
        df_log.to_csv(log_path, compression='gzip', index=False, encoding='utf-8')
        print(f"✅ df_log 전체 기록 완료 -> 경로: {log_path}")
    except Exception as log_err:
        print(f"⚠️ df_log 백업 중 경고 발생(정상 진행 가능): {log_err}")
        # 중요 데이터를 유실하지 않기 위해 개별 실험 폴더에 임시 백업 처리
        df_log_new.to_csv(outdir + 'df_log_local.csv.gz', compression='gzip', index=False, encoding='utf-8')