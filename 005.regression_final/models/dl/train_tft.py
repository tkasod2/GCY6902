...
def train(
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val < best - 1e-8:
            best = val
            bad = 0
            torch.save({"cfg": asdict(cfg), "state_dict": model.state_dict()}, save_path+'tft_best.pt')
            print(f" -> saved best to {save_path}tft_best.pt")
        else:
            bad += 1
            if bad >= patience:
    ...
    checkpoint = torch.load(save_path+'tft_best.pt', map_location=device)
...
def train_lstm(
    ...
    for epoch in range(1, epochs + 1):
        ...
        epoch_dir_lstm = save_path
        # torch.save(model.state_dict(), epoch_dir_lstm + f'lstm_{epoch}.pt')
        if val < best - 1e-8:
            best = val
            bad = 0
            torch.save(model.state_dict(), save_path+'lstm_best.pt')
            print(f" -> saved best LSTM to {save_path}lstm_best.pt")
        else:
            bad += 1
            if bad >= patience:
                print(" -> early stopping")
    ...
    # 최고 성능 가중치 로드
    model.load_state_dict(torch.load(save_path+'lstm_best.pt', map_location=device))
    return model, df_epoch
