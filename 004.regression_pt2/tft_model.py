...
class VariableSelectionNetwork(nn.Module):
    ...
    def __init__(self, n_vars: int, d_model: int, hidden_dim: int, dropout: float = 0.1, ctx_dim: int = 0):
        ...
        self.weight_grn = GRN(in_dim=n_vars * d_model, out_dim=n_vars,
                             hidden_dim=hidden_dim, dropout=dropout, ctx_dim=ctx_dim)
        # per-variable GRN
        self.var_grns = nn.ModuleList([GRN(d_model, d_model, hidden_dim, dropout=dropout) for _ in range(n_vars)])

    def forward(self, var_inputs: torch.Tensor, context: Optional[torch.Tensor] = None) \
...
class TemporalFusionTransformer(nn.Module):
    ...
    - x_known: (B, L, known_vars)    known covariate (시간정보)
    - x_static: (B, static_vars) or None 정적 변수(산업코드)
    - pos: (B,) or None             Position
    ...
    def __init__(self, cfg: TFTConfig):
        ...
        self.known_projs = nn.ModuleList([nn.Linear(1, d) for _ in range(cfg.known_vars)]) \
            if cfg.known_vars > 0 else nn.ModuleList()
            
        self.static_projs = nn.ModuleList([nn.Linear(1, d) for _ in range(cfg.static_vars)]) \
            if cfg.static_vars > 0 else nn.ModuleList()
            
        # 2) static context encoders (optional)
        # TFT는 static context를 여러 곳(GRN, VSN)에 넣는데, 여기서는 간단히 1개 context로 사용
        self.static_context_grn = GRN(in_dim=d, out_dim=d, hidden_dim=cfg.hidden_dim,
                                     dropout=cfg.dropout) if cfg.static_vars > 0 else None
                                     
        # 3) Variable selection networks
        ctx_dim = d if cfg.static_vars > 0 else 0
        self.vsn_past = VariableSelectionNetwork(cfg.past_vars, d, cfg.hidden_dim,
                                               dropout=cfg.dropout, ctx_dim=ctx_dim)
                                               
        self.vsn_known = \
            VariableSelectionNetwork(cfg.known_vars, d, cfg.hidden_dim,
                                   dropout=cfg.dropout, ctx_dim=ctx_dim) if cfg.known_vars > 0 else None
        ...
        self.out = nn.Linear(d, len(cfg.quantiles))
        self.out_cls = nn.Linear(d, 3)              # 🎆🎇260501 방향성 예측용 (0:유지, 1:상승/악화, 2:하락/개선)
    ...
    def forward(
        ...
        # Dual output heads (회귀/분류 + 방향성) # 🎆🎇260501
        out_reg = self.out(y_sel)
        out_cls = self.out_cls(y_sel) # 🎆🎇260501 방향성 예측 (3-class) 
        ...
        # return out, aux
        return (out_reg, out_cls), aux # 🎆🎇260501
