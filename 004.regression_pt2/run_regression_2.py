...
def fit_and_out(df,
    ...
    * 기타
    ...
    if train_yn==1: # 학습만 수행시
        ...
        df_epoch = pd.read_csv(outdir+'fdf_epoch.csv.gz')
        df_config = pd.DataFrame.from_dict({'keys': config_dict.keys(),'values': config_dict.values()})
        model, model_lstm, df_epoch, df_config = fit_model(config_dict,epochs=epochs,patience=patience)
    ...
    with torch.no_grad():
        ...
        for x_p, x_k, x_s, p, y_reg, y_cls in ts_loader:
            ...
            all_tft_q10.append(y_tft_reg[:, 0].numpy())
            all_tft_q50.append(y_tft_reg[:, 1].numpy()) # 이게 기존의 all_tft_preds 역할 (중앙값)
            all_tft_q90.append(y_tft_reg[:, 2].numpy())

            # --- 기존 로직 완벽 유지 ---
            all_actuals.append(y_reg.numpy())
            all_lstm_preds.append(y_lstm.numpy())
            all_weights.append(aux['w_past'].numpy())             # 변수 중요도
            all_attention_score.append(aux['attn_w'].numpy())     # 어텐션 스코어
